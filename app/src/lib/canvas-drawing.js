function mapNodes(nodes) {
  ...
  var depthSize = 80,
    ...
    notPath = new Path2D(
      "M 28.9688 2.5938 v 44.4062 l 27.0312 -22 L 56 25 A 1 1 0 0 0 63 25 A 1 1 0 0 0 56 25 z"
    ),
    ...
    inputPath = new Path2D(
        "m 141.75 138.6 v 20.25 l 14.85 0 l 9.45 -10.8 l -9.9 -9.45 z"
    ),
    wireColor = "#093708"
  ...
  function positionNodes(t, e, n) {
      ...
      t.leftInput && t.rightInput
        ...
        : "INPUT" == t.circutType && ((t.x = -600), (t.y = -100));
  }
  ...
  function drawLogicGates(node, ctx) {
      ...
      switch (node.circutType) {
        ...
        ctx.save();
          let x =  getSuitableX(node) - 95
        //   let y = node.inputVaraible == 'A' ? 100 : node.inputVaraible == 'B' ? 200 : node.inputVaraible == 'C' ? 300 : node.inputVaraible == 'C' ? 400 : -1500;
          let y = getSuitableY(node) - 124
          ctx.fillStyle = "#0300c3"
          ctx.strokeStyle = "#060352"
        //   ctx.scale(0.4, 0.4)
          ctx.translate(x, y);
          ctx.fill(inputPath);
          ctx.stroke(inputPath);
          ctx.restore();
          
          ctx.fillStyle = "#ffffff"
          ctx.font = "12px Arial"
          ctx.fillText("0", x + 148 , y + 153);
        //   ctx.beginPath();
        //   ctx.arc(x + 50, y + 25, 20, 0, 2 * Math.PI);
        //   ctx.lineWidth = 3;
        //   ctx.stroke();  
        ...
        case "OUTPUT":
            ...
            let _x = 30, _y = 15
            ctx.beginPath();
            ctx.rect(node.x + _x, node.y + _y, 20, 20)
            ctx.lineWidth = 2
            ctx.fill();
            ctx.stroke();
            
            ctx.save();
            ctx.fillStyle = "#6d0703"
            ctx.font = "bold 16px Arial"
            ctx.fillText("?", node.x + _x + 5, node.y + _y + 15);
            ctx.restore();
      ...
      if (node.leftInput) {
        ...
        if (node.leftInput.circutType !== 'INPUT') {
            drawBetweenNodes(node.leftInput, node, "leftInput", ctx);
                ...
                drawBetweenNodes({x: getSuitableX(node.leftInput), y: getSuitableY(node.leftInput) , circutType: 'INPUT'}, node, "leftInput", ctx, getSuitableDistanceFromInput(node.leftInput));
        }
      }
      ...
      if (node.rightInput) {
        ...
        if (node.rightInput.circutType !== 'INPUT') {
          ...
          drawBetweenNodes({x: getSuitableX(node.rightInput), y: getSuitableY(node.rightInput) , circutType: 'INPUT'}, node, "rightInput", ctx, getSuitableDistanceFromInput(node.rightInput));
        }
      }
      ...
      if (node.input) {
        ...
        if (node.input.circutType !== 'INPUT') {
          ...
          drawBetweenNodes({x: getSuitableX(node.input), y: getSuitableY(node.input) , circutType: 'INPUT'}, node, "input", ctx, getSuitableDistanceFromInput(node.input));
        }
      }
  }
  ...
  function drawBetweenNodes(n1, n2, inputDir, ctx, firstAbsoluteXDistance) {
      ...
      if ((n1.circutType == "INPUT" && n2.circutType == "NOT") ||
        ...
        } else if (n1.circutType == "NOT" && n2.circutType == "AND" ||
                   n1.circutType == "NOT" && n2.circutType == "OR")  {
          drawLine(x1-6, y1, x2, y2, ctx, firstAbsoluteXDistance);
        } else if (n2.circutType === 'OUTPUT') {
          drawLine(x1, y1, x2, y2, ctx, firstAbsoluteXDistance);
      }
  }
